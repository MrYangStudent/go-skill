...
├── log.md                # 操作日志（追加式记录每次 add/reindex/lint 操作）
...
| 时机 | 强制操作 | 对应命令 |
|------|----------|----------|
| **编码前（必须）** | 搜索知识库 → 检查条目时效性 → lint 检查 → 优先复用已有 | `knowledge search <关键词>` + `knowledge check-stale .` + `knowledge lint` |
...
| 扫描代码 | 自动提取项目中的导出函数和 GoDoc 注释 | `knowledge scan [目录...]` |
| 批量检查时效 | 对比来源文件修改时间 | `knowledge check-stale <项目目录>` |
| 完整性检查 | 检查重复标题、弱标签、过旧条目、索引不一致 | `knowledge lint` |
| 发现关联 | 自动推荐条目间的关联（import分析 + 标签相似度） | `knowledge link [项目目录]` |

### 强制规则

1. **编码前必查**：读取 INDEX.md → `knowledge search <关键词>` → `knowledge check-stale .` → `knowledge lint` → 在开发计划中列出复用条目
2. **编码后必存**：识别新增可复用模式 → 去重 → `knowledge add` 保存 → 报告结果。无新增则说明"无可复用内容"

### 知识条目格式

支持两种格式，**向后兼容**：

#### 格式一：Markdown 字段格式（默认，兼容已有项目）
...
#### 格式二：YAML frontmatter 格式（推荐新条目使用，结构化更规范）

```markdown
---
id: entry-1719254400
title: "[search, slice] SearchUtils - 通用切片搜索函数"
tags: [search, slice, filter]
source: "pkg/slicex/search.go"
updated: 2026-06-25
purpose: "在切片中快速搜索符合条件的首个元素"
...
> **格式选择**：`knowledge add` 默认使用格式一（兼容旧项目）；使用 `knowledge add --format yaml` 生成格式二。两种格式的条目可以共存于同一个知识库，`search`/`list`/`check-stale`/`reindex`/`lint` 均支持双格式解析。
...
# 3. 开始添加知识条目（格式一：默认兼容旧格式）
knowledge add -t "[page] 功能名 - 简述" -s "pkg/xxx/xxx.go" -p "用途描述"

# 4. 添加 YAML 格式条目（格式二：推荐新条目使用）
knowledge add --format yaml -t "[page] 功能名 - 简述" -s "pkg/xxx/xxx.go" -p "用途描述"

# 5. 添加带关联的知识条目
knowledge add --format yaml -t "[search] SearchUtils - 搜索" -s "pkg/search.go" -p "搜索列表" -r "FilterUtils - 过滤" "SortUtils - 排序"
